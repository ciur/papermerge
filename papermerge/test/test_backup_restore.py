...
class TestBackupRestore(TestCase):
    ...
    def test_backup_single_document(self):
        ...
        with io.BytesIO() as memoryfile:
            ...
            self.assertIs(len(backup_info.get('documents')), 1,
    ...
    def test_backup_document_hierachy(self):
        folder_1 = Folder.objects.create(title='1', parent=None, user=self.testcase_user)
        folder_2 = Folder.objects.create(title='2', parent=folder_1, user=self.testcase_user)
        folder_3 = Folder.objects.create(title='3', parent=folder_1, user=self.testcase_user)
        folder_4 = Folder.objects.create(title='4', parent=None, user=self.testcase_user)
        ...
        with io.BytesIO() as memoryfile:
            ...
            self.assertIsNotNone(backup_info.get('documents'), 'backup.json did not have a key "documents"')
            self.assertIs(len(backup_info.get('documents')), 2,
                          'backup.json key documents had more or less than two entry')
            self.assertIs(len(backup_file.getnames()), 3, 'backup.tar had more or less than 2 entries')
            self.assertTrue('1/2/berlin.pdf' in backup_file.getnames(),
                            'berlin.pdf was not in the backup.tar at folder 1/2/')
            self.assertTrue('1/3/berlin.pdf' in backup_file.getnames(),
